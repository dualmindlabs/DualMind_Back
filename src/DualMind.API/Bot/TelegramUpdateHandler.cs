...
namespace DualMind.API.Bot
{
    public class TelegramUpdateHandler
    {
        ...
        public async Task HandleAsync(TelegramIncomingUpdate update, CancellationToken cancellationToken)
        {
            ...
            {
                await _transport.SendTextMessageAsync(update.ChatId, "⚠️ *Invalid Input*\n\nPlease send text messages only in this version of the bot\\.", null, cancellationToken);
                return;
            }
        ...
        private async Task HandleCallbackAsync(TelegramIncomingUpdate update, CancellationToken cancellationToken)
        {
            ...
            await _transport.SendTextMessageAsync(update.ChatId, "❓ *Unknown Action*\n\nI'm not sure how to handle that button click\\. Try using /start to reset\\.", null, cancellationToken);
        }
        ...
        private bool TryHandleCommand(long chatId, string text, CancellationToken cancellationToken, out Task? commandTask)
        {
            ...
            switch (command.ToLowerInvariant())
            {
                ...
                case "/battle":
                    ...
                    {
                        commandTask = _battleCommandHandler.HandleCommandAsync(chatId, cancellationToken);
                    }
                    else
                    {
                        commandTask = _battleCommandHandler.HandlePromptAsync(chatId, arguments, cancellationToken);
                    }
        ...
        private async Task BeginSignInAsync(long chatId, CancellationToken cancellationToken)
        {
            ...
            if (session != null)
            {
                await _transport.SendTextMessageAsync(
                    chatId,
                    "✅ *Already Signed In*\n\nYou are already authenticated\\. Use /battle to start\\!",
            ...
            await _transport.SendTextMessageAsync(
                chatId,
                "👋 *Welcome\\!*\n\nI'm ready to find the best AI for you\\. Please send the *email address* you use for DualMind Arena\\.",
                null,
                cancellationToken);
        }
        ...
        private async Task HandleEmailAsync(long chatId, string email, CancellationToken cancellationToken)
        {
            ...
            {
                await _transport.SendTextMessageAsync(chatId, "❌ *Invalid Email*\n\nThat doesn't look like a valid email\\. Please try again\\.", null, cancellationToken);
                return;
            }
            ...
            await _transport.SendTextMessageAsync(
                chatId,
                "🔐 *Password Required*\n\nNow send your password\\. I will delete that message *instantly* after reading it for your security\\.",
                null,
                cancellationToken);
        }
        ...
        private async Task HandlePasswordAsync(long chatId, int messageId, string password, CancellationToken cancellationToken)
        {
            ...
            {
                _stateCache.SetAwaitingEmail(chatId);
                await _transport.SendTextMessageAsync(chatId, "🔄 *Session Restarted*\n\nLet's start over\\. Please send your *email address*\\.", null, cancellationToken);
                return;
            }
            ...
            try
            {
                ...
                await _transport.SendTextMessageAsync(
                    chatId,
                    "✨ *Sign\\-in Successful\\!*\n\nYou're all set\\. Use /battle to start a blind AI comparison or check the /stats\\.",
                    TelegramMessageFormatter.BuildMainMenuKeyboard(_options.SignupUrl),
                    cancellationToken);
            }
            catch (TelegramAuthException ex)
            {
                await _transport.SendTextMessageAsync(
                    chatId,
                    $"❌ *Sign\\-in Failed*\n\n{TelegramMessageFormatter.EscapeMarkdown(ex.Message)}\n\nPlease send your password again or use /start to restart\\.",
                    null,
                    cancellationToken);
            }
        }
