...
namespace DualMind_Back.Controllers.Admin
{
    ...
    public class ProvidersController : ApiController
    {
        ...
        [HttpPost]
        [Route("providers")]
        public async Task<IHttpActionResult> CreateProvider([FromBody] ProviderCreateRequest request)
        {
        ...
        public async Task<IHttpActionResult> UpdateProvider(string name, [FromBody] ProviderUpdateRequest request)
        {
        ...
        public async Task<IHttpActionResult> GetProviderKeys(string name)
        {
            try
            {
                // Ensure fresh data
                ...
                // Security: Ensure EncryptedApiKey is NOT returned or is null
                // Our DTO has [JsonIgnore] on EncryptedApiKey but just to be safe:
                ...
                return Ok(new { success = true, data = safeKeys });
            }
            catch (Exception ex)
            {
                return Content(HttpStatusCode.InternalServerError, new { success = false, error = ex.Message });
        ...
        [HttpPost]
        [Route("providers/{name}/keys")]
        public async Task<IHttpActionResult> AddProviderKey(string name, [FromBody] ProviderApiKeyCreateRequest request)
        {
            ...
            try
            {
                ...
                var newKey = new ProviderApiKey
                {
                    ...
                    CreatedAt = DateTime.UtcNow,
                    UpdatedAt = DateTime.UtcNow,
                    // CreatedBy = GetUserIdFromToken() // Implement via base class or extension later
                };
                
                // We send a smaller object to Supabase that matches the schema
                // (DTO has extra props or ignore attributes that might interfere if we serialize directly)
                // Using an anon object for insertion ensures we map precisely.
                var dbObj = new 
                {
                    key_id = newKey.KeyId,
                    provider_name = newKey.ProviderName,
                    encrypted_api_key = newKey.EncryptedApiKey,
        ...
        public async Task<IHttpActionResult> UpdateKeyStatus(string id, [FromBody] ProviderApiKeyStatusUpdate request)
        {
        ...
        [HttpDelete]
        [Route("keys/{id}")]
        public async Task<IHttpActionResult> DeleteKey(string id)
        {
